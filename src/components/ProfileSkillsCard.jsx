...
export default function ProfileSkillsCard({ skills }) {
  return (
    ...
    >
        ...
        {(skills.chips || []).map((c) => (
          ...
          >
            ...
            {(c.level !== undefined || c.years !== undefined) && (
              <span
                style={{ fontSize: "0.7rem", opacity: 0.7, marginLeft: "6px" }}
              >
                {c.level !== undefined &&
                  `• ${LEVEL_LABEL[c.level] ?? c.level}`}
                {c.years !== undefined &&
                  ` • ${c.years} yr${c.years !== 1 ? "s" : ""}`}
              </span>
            )}
            {/* Delete button — only shown if handler exists */}
            {skills.onDelete && (
              <button
                type="button"
      ...
      {/* Add Skill button — only shown if handler exists (own profile) */}
      {skills.onAdd && (
        <div
          style={{
            display: "flex",
